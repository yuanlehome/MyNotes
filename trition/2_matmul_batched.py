...
def matmul_kernel(
    ...
    a_ptrs = (
        a_ptr
        + a_batch_stride * batch_idx
        + (a_offs[:, None] * a_m_stride + k_range_offs[None, :] * a_k_stride)
    )
    b_ptrs = (
        b_ptr
        + b_batch_stride * batch_idx
        + (k_range_offs[:, None] * b_k_stride + b_offs[None, :] * b_n_stride)
    )
    ...
    c_ptrs = (
        c_ptr
        + c_batch_stride * batch_idx
        + c_m_stride * c_m_offs[:, None]
        + c_n_stride * c_n_offs[None, :]
    )
